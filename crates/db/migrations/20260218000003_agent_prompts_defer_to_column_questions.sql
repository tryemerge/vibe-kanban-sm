-- Agent prompts should NOT hardcode answer values like "complete".
-- Columns define their own question + answer_options, and build_decision_instructions()
-- dynamically appends those to the agent prompt. The base prompt should tell the agent
-- to follow the Question section's answer options when present.

-- Strategic Planner: remove hardcoded "complete", defer to Question section
UPDATE agents SET
...
Write your plan and answer to `.vibe/decision.json`. If a **Question** section appears at the end of this prompt, use the answer options specified there. Otherwise, write:
...
-- Software Developer: remove hardcoded "complete", defer to Question section
UPDATE agents SET
...
2. Write your completion status to `.vibe/decision.json`. If a **Question** section appears at the end of this prompt, use the answer options specified there. Otherwise, write:
...
-- Code Reviewer: already uses approve/request_changes which match column options,
-- but add the same "defer to Question section" pattern for consistency
UPDATE agents SET
...
Write a review summary and your decision to `.vibe/decision.json`. If a **Question** section appears at the end of this prompt, use the answer options specified there. Otherwise:
...
-- Task Planner: remove hardcoded "complete", defer to Question section
UPDATE agents SET
...
Write your execution brief as a context artifact to `.vibe/decision.json`. If a **Question** section appears at the end of this prompt, use the answer options specified there. Otherwise, write:
...
You are not writing code. You are making it possible for someone else to write code confidently and correctly.',
    start_command = 'Analyze this task. Review the codebase, evaluate available context, and produce a clear execution brief. Write your findings as an artifact and your answer to .vibe/decision.json.',
    updated_at = NOW()
